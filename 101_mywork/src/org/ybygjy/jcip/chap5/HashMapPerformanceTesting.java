...
public class HashMapPerformanceTesting {
	...
	public void doTest() throws InterruptedException {
		...
		final Random random = new Random(
				(int) (this.threadNums * this.loopNums * this.rwRatio));
		for (int i = 1; i <= threadNums; i++) {
			new Thread("HMTThread_"
					+ targetContainer.getClass().getSimpleName() + "_" + i) {
				@Override
				public void run() {
					try {
						beginGate.await();
	...
	public String toString() {
		return "HashMapPerformanceTesting [threadNums=" + threadNums
				+ ", loopNums=" + loopNums + ", rwRatio=" + rwRatio
