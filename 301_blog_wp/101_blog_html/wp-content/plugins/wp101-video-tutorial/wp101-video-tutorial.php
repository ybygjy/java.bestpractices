...
class WP101_Video_Tutorial {
	...
	public function api_key_error_message() {
		echo '<div class="updated"><p>' . __( 'You are accessing this content from an invalid IP address. Please uninstall this plugin right away.', 'wp101' ) . '</p></div>';
	}
...
<div class="wrap" id="wp101-settings">
	...
	<?php if ( isset( $_GET['configure'] ) && $_GET['configure'] ) : ?>
		...
		<?php else : ?>
			<p><?php _e( '<strong class="wp101-valid-key">WP101 is ready to go!</strong>', 'wp101' ); ?>
		<?php endif; ?>
	<?php endif; ?>
...
<?php if ( $document_id ) : ?>
	...
	<?php if ( $document ) : ?>
		...
		<?php //echo do_shortcode( '[video mp4="https:' . $document["content"] . '"]' ); ?>
	<?php else : ?>
...
<?php else : ?>
	<p><?php _e( 'No help topics found. Please try again later.', 'wp101' ); ?></p>
<?php endif; ?>
